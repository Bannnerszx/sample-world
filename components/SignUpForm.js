...
import React, { useEffect, useState, useRef, useCallback, useMemo, useReducer, useContext } from 'react';
...
const SignUpForm = () => {
    ...
    useEffect(() => {
        const fetchCarData = async () => {
            const vehicleDocRef = doc(projectExtensionFirestore, 'CustomerCountryPort', 'CountriesDoc');
            ...
            try {
                ...
                if (docSnapshot.exists()) {
                    ...
                    // Extract only the country names and replace underscores with periods
                    const countryNames = Object.keys(data).map((country) => country.replace(/_/g, '.'));

                    console.log('Country Names:', countryNames);

                    // Set the country names array into the state
    ...
    const handleNext = async () => {
        ...
        if (step === 1) {
            ...
            if (emailIsEmpty) {
                ...
                if (!isEmailError) {
                    try {
                        const methods = await fetchSignInMethodsForEmail(projectExtensionAuth, email);
    ...
    console.log('COUNTRY', selectedCountry ? selectedCountry.value : [])
    console.log('text phone number', textPhoneNumber)

    //Create an account start
    const handleSubmit = async () => {
        const isCountryBlank = !selectedCountry || selectedCountry.value === '';
        const isCityBlank = !selectedCity || selectedCity.label === '';
        const isStreetBlank = !textStreetRef.current || textStreetRef.current.trim() === '';
        const isZipBlank = !textZipRef.current || textZipRef.current.trim() === '';
        const isPhoneBlank = !textPhoneNumber || textPhoneNumber.trim() === '';

        // Check for errors or blank inputs
        if (countryError || cityError || textStreetError || textZipError || textPhoneNumberError || isCountryBlank || isCityBlank || isStreetBlank || isZipBlank || isPhoneBlank) {
            if (!isCountryBlank) {
                setCountryError(false)
            } else {
            ...
            // if (!isCityBlank) {
            //     setTextStreetError(true);
            // } else {
            //     setTextStreetError(false);
            // }
            if (!isStreetBlank) {
                setTextStreetError(false)
            } else {
            ...
            if (!isZipBlank) {
                setTextZipError(false)
            } else {
                setTextZipError(true);
            }
            if (!isPhoneBlank) {
                setTextPhoneNumberError(false)
            } else {
                setTextPhoneNumberError(true)
            }
            console.error("Please correct the errors and fill all required fields before submitting.");
            return;
        }
        setIsLoading(true);
        const countryLabel = selectedCountry ? selectedCountry.value : '';
        const cityLabel = selectedCity ? selectedCity.label : '';
        const street = textStreetRef.current;
        const zip = textZipRef.current;
        try {
            const email = textEmail.toLocaleLowerCase();
            ...
            await projectExtensionAuth.signOut(userAuth);

            // Get the authentication token



            const userDocRefAuth = doc(projectExtensionFirestore, 'accounts', email)
                                                    ...
                                                    <View style={{ height: 500 }}>
                                                        ...
                                                        <View style={{ zIndex: 1, width: '100%', marginVertical: 10, justifyContent: 'center', alignItems: 'center', alignSelf: 'center' }}>
                                                            ...
                                                            <PhoneInput
                                                                ...
                                                                specialLabel={''}
                                                                country={'jp'}
                                                                containerStyle={{ alignItems: 'center', justifyContent: 'center', alignSelf: 'center', width: '90%' }}
                                                                inputStyle={{ width: '100%', height: '45px', alignItems: 'center', justifyContent: 'center', alignSelf: 'center' }}
                                                            />
                                            ...
                                            {step === 3 && (
                                                <PresenceTransition visible={!isOpen} initial={{
                                                    ...
                                                    <View style={{ height: 500 }}>
                                                        ...
                                                        <View style={{ zIndex: 1, width: '90%', marginVertical: 10, justifyContent: 'center', alignItems: 'center', alignSelf: 'center' }}>
                                                            <PhoneInput
                                                                value={textPhoneNumber}
                                                                onChange={handlePhoneNumberChange}
                                                                specialLabel={''}
