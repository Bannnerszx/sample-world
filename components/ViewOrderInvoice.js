...
const ViewOrderInvoice = () => {
    ...
    useEffect(() => {
        const bookingListDocRef = doc(collection(projectExtensionFirestore, 'BookingList'), chatId);
        const chatRef = doc(projectExtensionFirestore, 'chats', chatId);

        const unsubscribe = onSnapshot(bookingListDocRef, (snapshot) => {
            if (snapshot.exists()) {
                const invoiceNumberData = snapshot.data()?.DocumentsUpload?.InvoiceNumber;
                setInvoiceNumber(invoiceNumberData || ''); // Set the InvoiceNumber or an empty string if it doesn't exist
            } else {
                console.log('Document does not exist');
                setInvoiceNumber(''); // Reset the InvoiceNumber if the document doesn't exist
            }
        });
        const unsubscribeChatField = onSnapshot(chatRef, (snapshot) => {
            if (snapshot.exists()) {
